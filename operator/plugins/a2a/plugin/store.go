...
const (
	...
	stateConfigMapName = "flokoa-a2a-plugin-state"

	// stateConfigMapLabel identifies the ConfigMap as managed by the plugin.
	stateConfigMapLabel = "flokoa.ai/component"
	stateConfigMapValue = "a2a-plugin-state"
)

// StateStore persists A2A task progress state across plugin restarts using a
...
// in-memory-only storage (fixes #97).
...
// taskKey recovers the original task key from a ConfigMap data key.
...
func (s *StateStore) loadFromConfigMap() {
	...
	if err := s.k8sClient.Get(context.Background(), key, cm); err != nil {
		if !apierrors.IsNotFound(err) {
			log.Printf("Warning: failed to load A2A plugin state ConfigMap: %v", err)
		}
		return
	}
	...
	if count > 0 {
		log.Printf("Restored %d A2A task state(s) from ConfigMap", count)
	}
}

// syncToConfigMap writes all cached state to the backing ConfigMap.
// The mutex serializes writes so concurrent Store/Delete calls coalesce
// into fewer ConfigMap updates (each sync captures the full current cache).
func (s *StateStore) syncToConfigMap() {
	...
	if err != nil {
		if !apierrors.IsNotFound(err) {
			log.Printf("Warning: failed to get A2A plugin state ConfigMap: %v", err)
			return
		}
		...
		if createErr := s.k8sClient.Create(context.Background(), cm); createErr != nil {
			log.Printf("Warning: failed to create A2A plugin state ConfigMap: %v", createErr)
		}
		return
	}
	...
	if updateErr := s.k8sClient.Update(context.Background(), cm); updateErr != nil {
		log.Printf("Warning: failed to update A2A plugin state ConfigMap: %v", updateErr)
	}
}
