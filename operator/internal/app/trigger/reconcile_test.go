...
func newBaseTrigger() *agentv1alpha1.AgentTrigger {
...
func parseTriggerConfig(t *testing.T, cms *fakes.FakeConfigMapRepo) trigger.TriggerConfig {
	t.Helper()
...
// assertCondition fails the test unless the condition exists with the given
// status. The reason is only checked when non-empty.
func assertCondition(t *testing.T, tr *agentv1alpha1.AgentTrigger, condType string, status metav1.ConditionStatus, reason string) {
	t.Helper()
	cond := findCondition(tr, condType)
	if cond == nil {
		t.Fatalf("expected %s condition", condType)
	}
	if cond.Status != status {
		t.Errorf("%s status = %s, want %s", condType, cond.Status, status)
	}
	if reason != "" && cond.Reason != reason {
		t.Errorf("%s reason = %s, want %s", condType, cond.Reason, reason)
	}
}

// reconcileTestCase describes a single table-driven Reconcile scenario.
...
func runReconcileTests(t *testing.T, tests []reconcileTestCase) {
	t.Helper()
...
func TestReconcileAgentResolution(t *testing.T) {
	runReconcileTests(t, []reconcileTestCase{
		{
			...
			verify: func(t *testing.T, result *trigger.ReconcileResult, tr *agentv1alpha1.AgentTrigger, cms *fakes.FakeConfigMapRepo) {
				...
				assertCondition(t, tr, triggerdomain.ConditionTypeAgentReady, metav1.ConditionTrue, "")
			},
		},
		{
			...
			verify: func(t *testing.T, result *trigger.ReconcileResult, tr *agentv1alpha1.AgentTrigger, _ *fakes.FakeConfigMapRepo) {
				...
				assertCondition(t, tr, triggerdomain.ConditionTypeAgentReady, metav1.ConditionFalse, triggerdomain.ReasonAgentNotFound)
				assertCondition(t, tr, triggerdomain.ConditionTypeReady, metav1.ConditionFalse, "")
			},
		},
		{
			...
			verify: func(t *testing.T, _ *trigger.ReconcileResult, tr *agentv1alpha1.AgentTrigger, _ *fakes.FakeConfigMapRepo) {
				assertCondition(t, tr, triggerdomain.ConditionTypeAgentReady, metav1.ConditionFalse, triggerdomain.ReasonAgentNotReady)
			},
		},
		{
	...
	runReconcileTests(t, []reconcileTestCase{
		{
			...
			wantPermanent: true,
			verify: func(t *testing.T, _ *trigger.ReconcileResult, tr *agentv1alpha1.AgentTrigger, _ *fakes.FakeConfigMapRepo) {
				assertCondition(t, tr, triggerdomain.ConditionTypeReady, metav1.ConditionFalse, triggerdomain.ReasonValidationFailed)
			},
		},
		{
...
func TestReconcilePushNotification(t *testing.T) {
	runReconcileTests(t, []reconcileTestCase{
		{
...
func TestReconcileTriggerConfig(t *testing.T) {
	runReconcileTests(t, []reconcileTestCase{
		{
