...
<template>
  <UDashboardPanel id="tools">
    ...
    <template #body>
      ...
      <EmptyState
        v-if="tools.length === 0 && status !== 'pending'"
        icon="i-lucide-wrench"
        title="No tools defined"
        description="Create AgentTool resources to give your agents access to external APIs via OpenAPI specifications."
        docs-url="https://flokoa.ai/agenttool"
        docs-label="Tool Guide"
      />
      ...
      <template v-else>
        <UTable
          ...
          @select="(_e: Event, row: { original: AgentTool }) => openDetail(row.original)"
        />
