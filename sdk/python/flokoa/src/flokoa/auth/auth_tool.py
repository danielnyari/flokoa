...
class AuthConfig(BaseModelWithConfig):
  ...
  """
    ...
    def get_credential_key(self):
        ...
        scheme_name = f"{auth_scheme.type_.name}_{_stable_model_digest(auth_scheme)}" if auth_scheme else ""
        ...
        credential_name = (
            f"{auth_credential.auth_type.value}_{_stable_model_digest(auth_credential)}" if auth_credential else ""
        )
