...
class OperationParser:
    ...
    def _process_operation_parameters(self):
        ...
        for param in parameters:
            if isinstance(param, Parameter):
                ...
                schema.description = description if not schema.description else schema.description
    ...
    def _process_request_body(self):
        ...
        for _, media_type_object in content.items():
            ...
            if schema and schema.type == "object":
                ...
                if (schema and (schema.oneOf or schema.anyOf or schema.allOf)) or not schema or not schema.type:
                    param_name = "body"
                else:
                    param_name = ""
    ...
    def _process_return_value(self) -> Parameter:
        ...
        valid_codes = list(filter(lambda k: k.startswith("2"), list(responses.keys())))
    ...
    def get_pydoc_string(self) -> str:
        ...
        pydoc_description = self._operation.summary or self._operation.description or ""
        pydoc_return = PydocHelper.generate_return_doc(self._operation.responses or {})
        pydoc_arg_list = chr(10).join(f"        {param_doc}" for param_doc in pydoc_params)
        return dedent(f"""
        \"\"\"{pydoc_description}
