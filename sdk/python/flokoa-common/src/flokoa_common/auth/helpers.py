...
"""Auth scheme/credential constructors and credential-to-parameter mapping.

Construction-time helpers for the OpenAPI auth layer: build ``AuthScheme`` /
``AuthCredential`` pairs from simple inputs, resolve OpenID Connect discovery
documents (with SSRF validation), and convert exchanged credentials into the
request parameter that carries them. Discovery helpers perform synchronous
HTTP — they are meant for one-shot configuration loading, not per-call paths
(per-call exchange lives in :mod:`flokoa_common.auth.exchangers`).
"""
...
"""Prefix for auth parameter py_names, keeping them out of model-visible schemas."""
...
def token_to_scheme_credential(
...
) -> tuple[AuthScheme, AuthCredential | None]:
    """Creates an AuthScheme and AuthCredential for an API key or bearer token.
    ...
    """
    if token_type == "apikey":  # noqa: S105 — scheme discriminator, not a secret
        in_: APIKeyIn
        if location == "header":
            in_ = APIKeyIn.header
        elif location == "query":
    ...
    elif token_type == "oauth2Token":  # noqa: S105 — scheme discriminator, not a secret
...
def _bearer_param_and_kwargs(
    ...
    kwargs = {py_name: f"Bearer {token}"}
    return param, kwargs


def credential_to_param(  # noqa: C901 — exhaustive scheme/credential dispatch, salvaged as-is
    ...
    # --- API Key ---
    # isinstance narrows the scheme union; APIKey is the only model carrying
    # type_ == apiKey (dict_to_auth_scheme constructs exactly it).
    if isinstance(auth_scheme, APIKey) and auth_scheme.type_ == AuthSchemeType.apiKey and auth_credential.api_key:
...
def resolve_openid_connect_scheme(
    ...
    return OpenIdConnectWithConfig.model_validate(
        {
...
def dict_to_auth_scheme(data: dict[str, Any]) -> AuthScheme:  # noqa: C901 — scheme-type dispatch, salvaged as-is
